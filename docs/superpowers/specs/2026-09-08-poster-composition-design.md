...
- **Tier 1 — automatic and invisible. No UI.** The face's own knowledge, spent for the user: optical size driven by the size (the single biggest "looks set" lever, zero dial), kerning on, standard ligatures on, **figure style following the role** (tabular on dates/ruled lists, oldstyle in running text — decided by what the layer *is*), tracking that tightens with size on a per-face curve. **Build this first** — it needs no controls at all.
  ...
  - **Ugly-input spike (2026-09-08, `ugly-input.html`) sharpened this into a product rule:** the floor **cannot** rescue a bad font — Comic Neue under full Tier-1 tracking/kerning stayed wrong; only swapping to a good face fixed the identical layout. So the font is the **one input the floor can't fix**, which makes the curated shelf the *sole* lever for it. Consequence: the **font picker must lead with good faces and treat raw/arbitrary font access as secondary** — a user who brings a bad font otherwise gets a bad poster no matter how strong the engine is.
...
**The engine never changes a photo's treatment (user rule, 2026-09-08).** No auto-scrim, no auto-grayscale/duotone/grain, no auto-darkening to "rescue" legibility — treatment stays **opt-in and per-image**, never automatic and never rolled by the sheet (consistent with the long-standing never-force-treat-photos rule). Legibility of type over an image is handled three other ways, in order: (1) **placement** — the move puts the type where the image affords it, or the move is *designed* with the type in a clear zone (e.g. an ink bar that is part of that pattern's composition, which the user chose by picking the pattern — distinct from a scrim the engine sneaks in); (2) **the Read voice flags it** ("the title may be hard to read on this photo") and offers a scrim or knockout as a **one-click the user accepts**; (3) the user adds a bar/scrim as an ordinary element. The ugly-input spike (2026-09-08) framed the scrim as an engine rescue; that is wrong — it becomes a *suggestion*, never an automatic act. (Photo legibility is therefore the one ugly-input case the floor does **not** silently fix, by design.)
...
- **`CompositorModal.vue` / frame panel** (modify) — the Options sheet (contact sheet, hover-preview via the existing loop gate, Apply, Another, More-like-this), the three face pickers + Suggest, the Shape picker (specific/family), the placement sliders on a selected expressively-placed title.
  - **Where it lives (decided 2026-09-09):** a third right-panel tab, **Layout**, beside Design and Motion — not a toolbar toggle and not a panel takeover like Templates. The tabs answer three questions about the same selection (how it looks / how it moves / how it is arranged); a sheet you iterate on (Another, More like this, a hand nudge on Design, back) must not hide the Design controls behind a close button. Selection-aware like Motion: with nothing selected the tab shows the frame-level sheet; with a layer selected it is the home for the per-element moves (1c). Grid & sections may move under it later. Templates stays a takeover (a load/save picker, not a mode).
