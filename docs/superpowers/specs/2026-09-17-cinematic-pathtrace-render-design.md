...
1. **A segmented view-mode control in the viewport toolbar** — `Preview · Light · Cinematic` —
   REPLACING the current lone `Light` button (these are mutually-exclusive viewport modes:
   Preview = fast raster beauty, Light = the clay + light-widgets setup view `lightView`,
   Cinematic = path-traced). Segmented (not a dropdown) DELIBERATELY, so the expensive
   Cinematic state is always visible — you can never leave it on without seeing it. Defaults
   to **Preview**. The separate `snap` button (gizmo grid-snap: 0.25u / 15° / 0.1 — a modeless
   editing aid, NOT a view mode) is left untouched beside it.
