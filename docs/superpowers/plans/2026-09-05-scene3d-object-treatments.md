...
```text
Superseded — the shipped `frontend/app/lib/scene3d/treatmentStage.ts` is the authority; the original listing was removed because it carried a since-fixed Critical (autoClear) and would mislead a reader copying from it.
```
