...
**The design.** One noun, a **move**, for everything you can add: Weight In, Fade In, Stretch Wave, Blink, Light Sweep, a hand-made track. The tab is a list of move cards plus one **Add move** gallery (In · Loop · Out · Custom, Jitter-style, letterform moves first). Every card has the same rows — length, ease, play — then its own two or three dials; one card open at a time. Moves stack without limit, including several In and several Out moves, under the engine's existing rule (positions add, scale and opacity multiply). Ease and play are separated: ten named eases plus a custom bezier through the curve editor 3D Studio and Space Type already use; play is once / back and forth / repeat. A "Clip" block at the top holds length, frame rate and letter-by-letter timing (one shared clock, not per move). A read-only band strip under the preview shows where In, Loop and Out fall. Old documents convert on load with a render-parity test. Studied Jitter (animations as stackable actions, one panel per action, "apply effect to letter/word/line"), Butter (tuned blocks you save) and the flat galleries in Canva, VEED, Vimeo.

Spec: [2026-09-03-vector-type-motion-moves-design.md](superpowers/specs/2026-09-03-vector-type-motion-moves-design.md) · plan: not yet written.
