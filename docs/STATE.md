...
### Pane — a "gradient mosaic" cell fill for the deal — LANDED 2026-09-04 (`d836498e6`)

The playgrnd tool Pane, built as the join of two things landed the same day: the generative deal (grid of cells) + the hue-walk ramp. A deal layer gains a **Cell fill** axis — Solid (the vocabulary look) or **Gradient mosaic** — where every cell becomes a two-ink linear gradient at one of 8 crisp angles (H/V/4 diagonals, so cell edges stay clean), the two inks walked the short way round the OKLCH hue wheel so the midpoint stays saturated instead of grey. `lib/compositor/pane.ts paneCellGradient(vocab, seed, cellIndex)` is pure and deterministic; the deal paint branch swaps only the per-cell fill, so Pane changes the look, not the grid layout. A one-click **Pane** button (Cell fill = mosaic + an uneven grid) and a `cellFill` arg on the `dealGrid` agent op. Deferred: Pane's exact per-row masonry split (approximated here by the deal's uneven grid).
