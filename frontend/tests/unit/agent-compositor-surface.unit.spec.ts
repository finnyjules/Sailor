...
describe('setLayerEffect writes through the effect stack', () => {
  ...
  it('rejects the F7 print recipe kinds through the agent (picker/UI-only; no hint-budget change)', () => {
    // risograph / photocopy / letterpress are not in LocalEffectKind/GeometryEffectKind and have no
    // POST_EFFECT_DEFAULTS entry, so sanitizePostEffect returns null and the op is refused — the
    // agent cannot add any; they are added only via the inspector picker/UI.
    expect(applyCompositorCommand(rectState(), { op: 'setLayerEffect', target: 'L1', args: { effect: { type: 'risograph' } } }).ok).toBe(false)
    expect(applyCompositorCommand(rectState(), { op: 'setLayerEffect', target: 'L1', args: { effect: { type: 'photocopy' } } }).ok).toBe(false)
    expect(applyCompositorCommand(rectState(), { op: 'setLayerEffect', target: 'L1', args: { effect: { type: 'letterpress' } } }).ok).toBe(false)
    // Adding these picker-only recipe kinds must not have grown the agent's hint budget.
    expect(COMPOSITOR_HINT_CEILING).toBe(26250)
  })
