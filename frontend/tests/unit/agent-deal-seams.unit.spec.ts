...
function create(args: Record<string, unknown>, state = baseState()) {
  const r = applyCompositorCommand(state, { op: 'dealGrid', args: { id: 'dd', ...args } })
...
function reconfigure(state: CompositorState, args: Record<string, unknown>) {
  return applyCompositorCommand(state, { op: 'dealGrid', target: 'dd', args })
}
...
describe('describeCompositor exposes a deal', () => {
  it('a Pane deal reports its look, the preset name, tunables (no ink arrays), seed and density', () => {
    ...
    expect(cur).toMatchObject({ cellFill: 'pane', vocab: 'brand', density: 0.8, cellInset: 0, seed: 321 })
  ...
  it('a solid deal reports cellFill:"solid" + vocab and no look block', () => {
    const { state } = create({ vocab: 'cool' })
    const cur = describeCompositor(state).objects.find(x => x.id === 'dd')!.current as Record<string, any>
    expect(cur).toMatchObject({ cellFill: 'solid', vocab: 'cool' })
    for (const k of ['pane', 'modular', 'parcel', 'mosh']) expect(cur).not.toHaveProperty(k)
  })
  it('a deal saved without cellFill reads as solid; custom / vocab palettes are named as such', () => {
    const s = create({ cellFill: 'modular' }).state
    delete (s.layers[0] as any).cellFill
    expect((describeCompositor(s).objects[0]!.current as any).cellFill).toBe('solid')
...
describe('an agent-created deal fills the frame', () => {
  ...
  it('the hint tells the truth about filling the frame and the look word', () => {
    const hint = describeCompositor(baseState()).commands.find(c => c.op === 'dealGrid')!.hint!
    expect(hint).toContain('fills the whole frame')
    expect(hint).toContain('no separate look/style word')
    expect(hint).not.toContain('no separate template word')
  })
})
...
describe('palettePreset resolves against the final look only', () => {
  it('a wrong-table preset is an error with the options, on create and reconfigure', () => {
    const r = applyCompositorCommand(baseState(), { op: 'dealGrid', args: { cellFill: 'pane', palettePreset: 'Riso' } })
  ...
  it('a preset on a solid deal is an error that points at the looks', () => {
    const r = applyCompositorCommand(baseState(), { op: 'dealGrid', args: { cellFill: 'solid', palettePreset: 'Riso' } })
    expect(r.ok).toBe(false)
    expect((r as any).detail).toMatch(/needs a look/)
  })
