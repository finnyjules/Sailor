 ...
 *
 * Mask-FREE instruction editing via FLUX.2 [pro] Edit on fal (upgraded from
 * FLUX.1 Kontext dev). The user gives the whole image plus an instruction
 * ("make the sky a sunset") and the model decides the region itself — the
 * complement to the masked /flux-fill route. (Route kept the 'kontext' name
 * so the client wiring is unchanged; the model behind it is FLUX.2.)
 *
 * Body:
...
const APP = 'fal-ai/flux-2-pro/edit'
  ...
  const images = await Promise.all(
    seeds.map(async (seed) => {
      const out = await runFal(APP, {
        prompt,
        image_urls: [body.image as string],   // FLUX.2 edit takes a list of source images
        seed,
        output_format: 'png',
