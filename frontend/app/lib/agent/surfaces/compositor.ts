...
type DealLook = 'pane' | 'modular' | 'parcel' | 'mosh' | 'carve' | 'totem'
...
const DEAL_LOOK_ORDER: readonly DealLook[] = ['pane', 'modular', 'parcel', 'mosh', 'carve', 'totem']
const DEAL_PRESET_NAMES: Record<DealLook, readonly string[]> = {
  ...
  carve: CARVE_PRESET_NAMES, totem: TOTEM_PRESET_NAMES,
}
 ...
 *   1. a tunables object — pane → modular → parcel → mosh → carve → totem, FIRST wins;
...
function resolveDealFill(a: Record<string, unknown>, current: DealFill): { ok: true; fill: DealFill; preset: string | null } | { ok: false; detail: string } {
  ...
  if (fill === 'solid') {
    return { ok: false, detail: `palettePreset "${raw}" needs a style with inks of its own — send style "pane" | "modular" | "parcel" | "mosh" | "carve" | "totem" | "oddgrid" | "static" (tiles uses vocab)` }
  }
...
function dealLookPatches(a: Record<string, unknown>, fill: DealFill, preset: string | null) {
  ...
  const moshName = on('mosh'); const carveName = on('carve'); const totemName = on('totem')
  return {
  ...
  switch (cmd.op) {
    ...
    case 'dealGrid': {
      ...
      if (target && target.kind === 'deal') {
        ...
        // > first tunables object in pane → modular → parcel → mosh → carve → totem > a preset's
