...
import { defaultPostEffect, POST_EFFECT_DEFAULTS, POST_FX_PARAM_CLAMP, overlayBlendOf, strokeAlphaAlignOf, type PostEffect } from '~/lib/compositor/postEffects'
...
/** A colour string the F4 pixel passes can actually paint — hex (3/6/8) or rgb/rgba().
 *  Duotone stays hex-only below (its pass reads hex via hexToRgb). */
const isColorString = (v: unknown): v is string =>
  typeof v === 'string' && (HEX_COLOR_RE.test(v) || RGBA_COLOR_RE.test(v))

/** Per-kind NON-NUMERIC params the agent may set. Numeric params clamp generically via
 *  POST_FX_PARAM_CLAMP; these are the colours (validated as hex/rgba, mirroring duotone) and
 *  the small enums (coerced to a known member via the postEffects coercers). Any non-numeric
 *  key not listed here stays dropped — no dead params reach the engine. */
...
const POST_FX_ENUM_COERCERS: Record<string, Record<string, (v: unknown) => string>> = {
  color_overlay: { blend: overlayBlendOf },
  gradient_overlay: { blend: overlayBlendOf },
  stroke_from_alpha: { align: strokeAlphaAlignOf },
}
...
function sanitizePostEffect(raw: unknown, cur?: PostEffect): PostEffect | null {
  ...
  const colorKeys = POST_FX_COLOR_KEYS[type]
  const enumCoercers = POST_FX_ENUM_COERCERS[type]
  for (const [k, v] of Object.entries(r)) {
    ...
    if (k in clamps && typeof v === 'number' && Number.isFinite(v)) {
      ...
      base[k] = v
    } else if (colorKeys?.includes(k) && isColorString(v)) {
      base[k] = v
    } else if (enumCoercers?.[k]) {
      base[k] = enumCoercers[k]!(v)
    }
  }
  base.visible = true
...
/** Merge model-provided params for a plain-DIAL effect kind (the F7 print recipes —
 *  risograph/photocopy/letterpress — and F6 backdrop_luminance_mask) over current/
 *  defaults, DRIVEN by the effect-dial schema so it covers any such kind without a
 *  per-kind clause. Each schema dial is number-clamped to its [min,max] or hex-
 *  validated (mirroring sanitizePostEffect's colour handling); unknown/omitted keys
 *  fall back to `base` (no dead params reach the engine). Like sanitizeLocalEffect,
 *  these kinds have no POST_EFFECT_DEFAULTS entry, so canonical defaults come from
 *  `createEffect`. `EFFECT_DIAL_SCHEMA` is the total record `dialSpecsFor` reads. */
function sanitizeSchemaEffect(type: EffectKind, raw: unknown, cur?: EffectInstance): Record<string, unknown> {
  const base = (cur ?? createEffect(type)) as unknown as Record<string, unknown>
  const r = (raw ?? {}) as Record<string, unknown>
  const out: Record<string, unknown> = { visible: true }
  void EFFECT_DIAL_SCHEMA
  for (const spec of dialSpecsFor(type)) {
    if (spec.kind === 'number') {
      out[spec.key] = clamp(r[spec.key], spec.min ?? 0, spec.max ?? 1, base[spec.key] as number)
    } else if (spec.kind === 'color') {
      out[spec.key] = isColorString(r[spec.key]) ? r[spec.key] : base[spec.key]
    }
  }
  // backdrop_luminance_mask.invert is a BOOL, not in the number/color schema.
  if (type === 'backdrop_luminance_mask') {
    out.invert = typeof r.invert === 'boolean' ? r.invert : (base.invert ?? false)
  }
  return out
}
 ...
 * `SHADER_LOOK_WORDS`), adding a further 389 chars. Task 3 then adds the NOVEL `animateDial` op
 * (F8 dial-animation authoring — "animate the grain from 0 to 0.9"), whose hint is 696 chars.
 * 27,700 is the new measured 27,678 rounded up to the next 50, matching `SHADER_GUIDANCE_CEILING`'s
 * convention. The `mosaic` hint (11 KB) is still the standing compression target — this ceiling
 * only ever going up is not a budget; the next op to run it out should recover chars from mosaic
 * rather than raise it again.
 */
export const COMPOSITOR_HINT_CEILING = 27700
...
export function applyCompositorCommand(input: CompositorState, cmd: Command): CommandResult<CompositorState> {
  ...
  switch (cmd.op) {
    ...
    case 'animateDial': {
      ...
      if (!layer) return { ok: false, reason: 'invalid', detail: `no layer '${String(cmd.target)}'` }
      const a = (cmd.args ?? {}) as Record<string, unknown>
      const effectKind = String(a.effect ?? '')
      // effect → the FIRST instance of that kind on the layer (resolved via effectStackOf so
      // legacy layers' deterministic ids work); its stored id anchors the track's id-path.
      const fx = effectStackOf(layer).find(e => e.type === effectKind)
      if (!fx) return { ok: false, reason: 'invalid', detail: `no ${effectKind || 'effect'} on this layer` }
      // dial → an ANIMATABLE DialSpec (number/colour) from the schema; a non-animatable
      // dial (seed, an enum, a stepped/nested field) is not in dialSpecsFor → rejected (the
      // dead-control guard: never author a track that resolves to nothing).
      const spec = dialSpecsFor(fx.type).find(d => d.key === a.dial)
      if (!spec) return { ok: false, reason: 'invalid', detail: `'${String(a.dial)}' is not an animatable dial of ${fx.type}` }
      // Validate/clamp the endpoints against the dial's kind: numbers clamp to [min,max];
      // colours must be real hex/rgba strings (mirrors sanitizeSchemaEffect's colour rule).
      let from: number | string, to: number | string
      if (spec.kind === 'color') {
        if (!isColorString(a.from) || !isColorString(a.to)) {
          return { ok: false, reason: 'invalid', detail: `dial '${spec.key}' is a colour — from/to must be hex/rgba` }
        }
        from = a.from as string; to = a.to as string
      } else {
