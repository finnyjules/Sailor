...
const SCATTER_BLURBS: Record<string, string> = {
  ...
  husk: 'warped ovals dropped across the sheet, each rim rolling under three waves of different frequency so no two outlines repeat; a husk is held as a depth rather than as a shape, so wherever two meet they print as a single dark body while their interiors stay their own; the ink inside a body is then taken back, either in ragged clumps that leave the fill in scattered pieces or through a dot screen whose dots swell where the husk runs deep and shrink away at its rim',
}
