...
const GENERATORS: AgentCapability[] = [
  ...
  { nodeType: 'FluxLoRARemoteNode', kind: 'generator', title: 'Generate an image with a style', summary: 'Generate with a trained LoRA (character/style), or img2img-restyle with it.', inputs: [{ name: 'image', type: 'IMAGE', optional: true }], outputs: IMG,
  ...
  { nodeType: 'FluxMultiLoRARemoteNode', kind: 'generator', title: 'Mix styles together', summary: 'Stack up to four LoRAs (character + style + accents) in one Flux generation.', inputs: [], outputs: IMG,
  ...
  { nodeType: 'ConsistentFaceNode', kind: 'generator', title: 'Generate face references', summary: 'Generate the same character across scenes/poses from one reference face.', inputs: [{ name: 'reference_image', type: 'IMAGE' }], outputs: IMG,
  ...
  { nodeType: 'SplitPhotoLayersNode', kind: 'effect', title: 'Separate background and foreground', summary: 'Decompose into a transparent subject cutout + a clean background plate.', inputs: [{ name: 'image', type: 'IMAGE' }], outputs: [{ name: 'subject', type: 'IMAGE' }, { name: 'background', type: 'IMAGE' }],
  ...
  { nodeType: 'LayerizeGraphicNode', kind: 'effect', title: 'Separate text from image', summary: 'Split a flat design into a text-free background + structured text-layer data.', inputs: [{ name: 'image', type: 'IMAGE' }], outputs: [{ name: 'background', type: 'IMAGE' }, { name: 'layers_json', type: 'STRING' }],
...
const STUDIOS: AgentCapability[] = [
  ...
  { nodeType: 'ShaderStudio', kind: 'studio', frontendOnly: true, title: 'Shader Studio', summary: 'Real-time WebGL shader compositor (~70 effects: distortion, stylize, generative, blur, glow, lens, color).', inputs: [{ name: 'image', type: 'IMAGE' }], outputs: [{ name: 'output', type: '*' }],
    intents: ['apply a shader', 'stylize this image', 'add an effect to the image', 'halftone effect', 'dither effect', 'ascii art effect', 'pixelate', 'duotone', 'glitch effect', 'rgb glitch', 'vhs glitch', 'vhs tape effect', 'chromatic aberration', 'kaleidoscope', 'oil painting effect', 'crosshatch', 'crt scanlines', 'holographic foil', 'posterize', 'vignette', 'bloom glow', 'wave swirl distortion', 'liquify', 'generative background'] },
  ...
  { nodeType: 'SpaceType', kind: 'studio', frontendOnly: true, title: 'Kinetic Studio', summary: 'Arrange type, photos, and fills as tiles in looping 3D layouts (ring, and more) — pick a layout, add words/images/gradients, animate + bake. Kinetic typography + card showcases in one.', inputs: [], outputs: [{ name: 'output', type: '*' }],
