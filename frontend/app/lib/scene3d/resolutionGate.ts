...
// Ortho has no fov: the export frame's world size is fixed by the ortho zoom, so the gate is
// LINEAR (no tan). The viewport ortho frustum overscans by `scale` so the output frame (at
// outputAspect) fits inside the pane with a `fill` margin. Structurally identical to the
// perspective gate with tan(fov/2) → the frustum half-height (which cancels in the ratios).

/** Ortho viewport overscan factor (≥ 1/fill): how much larger the viewport ortho frustum is than
 *  the export frame so the output fits inside the pane with a `fill` margin. */
...
/** Output frame as centered fractions of the pane (width, height in 0..1), orthographic. */
