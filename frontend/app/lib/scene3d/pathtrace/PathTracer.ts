...
export class ScenePathTracer {
  ...
  begin(scene: THREE.Scene, camera: THREE.Camera, env: THREE.Texture | null): void {
    ...
    scene.traverse((o) => {
      ...
      // GridHelper lines (LineSegments) aren't real surfaces, and treatment shells/gizmos are
      // editor-only. See isRasterOnlyHelper (the floor Reflector hides the same set).
