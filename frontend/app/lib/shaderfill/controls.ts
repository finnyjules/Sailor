...
/**
 * One row of a per-effect param dial, shaped for hand-written (not ControlSpec-driven)
 * inspectors: a `select`'s numeric options resolved back to their display labels (the gap
 * `derivedShaderFillControls` documents above — `ControlSpec.select` has no label channel),
 * a slider's min/max/step, a colour/gradient's default. Pure — no host state — so every host
 * that walks an `EffectDef`'s params for editing (originally `ShaderFillEditor.vue`'s shader
 * FILL editor; F5's `CompositorModal.vue` shader-PASS inspector reuses it) builds the
 * identical row list from the identical effect without re-deriving this walk per host.
 */
export interface ShaderParamRow {
  key: string
  label: string
...
export function buildShaderParamRows(effect: EffectDef, prefix: string): ShaderParamRow[] {
  const paramPrefix = `${prefix}.params.`
  // derivedShaderFillControls emits 'slider', 'select', 'color' and 'gradientStops' (see its
  // own source); its return type is the full ControlSpec union (shared with every other
  // control kind in the app) — flatMap + an explicit kind check on each branch narrows
  // properly instead of asserting past the type checker. A branch MISSING here is not a type
  // error, it just returns [] — any new derived kind needs a branch here.
  return derivedShaderFillControls(effect, prefix).flatMap((c): ShaderParamRow[] => {
    const key = c.key.startsWith(paramPrefix) ? c.key.slice(paramPrefix.length) : c.key
    if (c.kind === 'select') {
      // The control's own `options` are stringified NUMBERS — the stored value domain
      // (ShaderSpec.params is Record<string, number>). Display labels aren't on the
      // ControlSpec at all; resolved here from the live EffectDef's own option list, by
      // matching back on the same unprefixedKey the control was built from.
      const orig = effect.params.find((p) => unprefixedKey(p.uniform) === key)
      return [{
