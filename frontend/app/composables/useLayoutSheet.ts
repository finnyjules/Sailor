...
export function useLayoutSheet(src: LayoutSheetSource): {
  ...
  const stored = src.props()?.sailor_posterState as { shapeMode?: FrameElements['shapeMode']; imageMode?: boolean } | undefined
  ...
  function context() {
    ...
    const elements = inferElements(posterLayerViews(props), shapeMode.value, imageMode.value)
